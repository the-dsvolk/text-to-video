...
> **✨ This video was generated by our text-to-video system using Mochi-1 (state-of-the-art text-to-video generation)**
...
- **BentoML Inference Service (bento-service)**: A powerful Python service that runs the text-to-video pipeline on a GPU. It uses Genmo Mochi-1, a state-of-the-art text-to-video model that generates high-quality videos directly from text prompts. This service is managed by KServe for serverless autoscaling (including scale-to-zero).
...
sequenceDiagram
    ...
    alt No pods running (scale-to-zero)
        ...
        BS->>GPU: Load Mochi-1 model
        GPU-->>BS: Model loaded (24-30GB VRAM)
    ...
    BS->>GPU: Generate video with Mochi-1<br/>Prompt → 480x848 video, 31 frames
    GPU-->>BS: Return generated video

    BS->>BS: Export video with imageio<br/>H.264 codec, 12fps
...
- **⚡ GPU Acceleration**: Optimized for H100 with state-of-the-art Mochi-1 model
- **🎯 Production Ready**: Load balancing, health checks, and error handling
- **📊 High Performance**: Uses Mochi-1 for superior video quality and temporal consistency
...
- **Kubernetes Cluster**: With at least one GPU node (e.g., NVIDIA H100, A100, or RTX 4090 with 24GB+ VRAM).
